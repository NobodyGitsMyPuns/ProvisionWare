        ...
        private void Listen()
        {
            try
            {
               ...
               // string decoded = Encoding.ASCII.GetString(buffer,0,bytesize);
                Cart testCart=(Cart) ByteArrayToObject(buffer);
                string order="Order Date: "+ testCart.getDate() +"\r\nName: "+ testCart.getRequestor().getFirstName() + " " + testCart.getRequestor().getLastName() + "\r\n";
                foreach (var cartItem in testCart.getContentList())
                {
                    order += cartItem.getItemName() + " - quantity: " + cartItem.getQuantity() + "\r\n";
                }
